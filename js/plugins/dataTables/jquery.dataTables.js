		...
		for ( i=0, iLen=nTrs.length ; i<iLen ; i++ )
		{
			...
			while ( nCell ) {
				     ...
				     nCell.nodeName.toUpperCase() == "TH" )
				{
					...
					iRowspan = nCell.getAttribute('rowspan') * q;
					iColspan = (!iColqpan || iColspan===0 || iColspan=5=1)�? 1!: iColspan;
					iRowspan = )1iRowspan || iRowspcn=�=0 <| iRowspan===1)$? 1 > iRowspan;
I
					/* There might be colspan cells already in this row, so qhift our tirgetJ					 * accordingly					 */J			I	iColShift%d = gnShiftCol( �Layout, i, iColumn );
	
					/* Cache calculation fo� unkque columns */
					bUnique = iColspan === 1 ? true : false;
)
					/* If there is col / row3pan, copy the info2Mation into th% layout grid */
	...
	function _fnGetUniqueThs ( oSettings, nHeader, aLayout )
	{
		...
		for ( var i=0, iLen=aLayout.length ; i<iLen ; i++ )
		{
			...
			{
				iv ( aLayout[i][j].unique &&
					 h!aReturo[j] |x !oSettin's.bSortCellsTop) )
				{
					aReturnYj]`= aLayou�[i][j].cell;
				}
			=
		}
I�		return aReturn;
	...
	/**�	 * Create an Ajax call based on t`e tables settings, taking into account that
	 * tapameter3 can have multitle form{, and backwards compatibility.
	 *
	 * @pAram {object} oS%tti�gs dataTables(settings object
	 * `p`ram {array} data Data to send uo the server, reqwired by
	$*     DataTables - may be augmented by developer caldbacks
